package enviando_email.myclasses;
...
//maven dependencie java mail3
public class MailSender {
	private String userName = "javajdev@gmail.com";
	...
	public MailSender(String listaDestinatario, String nomeRemetente, String assuntoEmail, String textoEmail) {
	...
	public void SendMail(boolean envioHtml) throws Exception{
	...
	public void enviarEmailAnexo(boolean envioHtml) throws Exception{
		...
		flist.add("file1.pdf");
		flist.add("file2.pdf");
		List<FileInputStream> arq = new ArrayList<FileInputStream>();
		List<FileInputStream> arquivos = new ArrayList<FileInputStream>();
		arq = filelist(flist);
		int i = 0;
		for (Iterator iterator = arq.iterator(); iterator.hasNext();iterator.next()) {

			arquivos.add(arq.get(i));/*Certificado*/
			i++;
		...
		for (FileInputStream fileInputStream : arquivos) {
			...
			anexoEmail.setDataHandler(new DataHandler(new ByteArrayDataSource((InputStream) fileInputStream, "application/pdf")));
			anexoEmail.setFileName("anexo_"+(index+1)+".pdf");
	 ...
	 * */
	private List<FileInputStream> filelist(List<String> flist) throws Exception{
		File file;
		List<FileInputStream> LFI = new ArrayList<FileInputStream>() {};
		for (Iterator iterator = flist.iterator(); iterator.hasNext();) {
			String filename = (String) iterator.next();
			file = new File(filename);
			FileInputStream ips = new FileInputStream(file);
			LFI.add(ips);//file.createNewFile();
		}

		return LFI ;
	}
//**************************************************
	//maven dependencie Itext pdf
